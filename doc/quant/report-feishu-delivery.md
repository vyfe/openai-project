...
操作登记要求飞书 open_id 已绑定量化用户。机器人会先生成待确认记录，只有收到确认命令后才写入 `QuantOperationRecord`；操作登记不会自动修改 `QuantPositionJournal`。
...
交易日可以省略，省略时默认当天；`买入/加仓/减仓/卖出`必须填写数量，`观察`可以不填写数量。群聊中需要明确 @机器人，且群 `chat_id` 必须配置在启用的 IM 通道中。
