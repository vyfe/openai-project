...
- 量化模块必须使用独立 SQLite 库
- 不能复用当前 `log.sqlite3_file` 指向的主业务日志库
- 不能共享现有 `logs.db` 的归档、清理、恢复脚本生命周期

原因很直接：

- 当前项目已有面向 `log/dialog` 的归档与删除脚本
- 这些脚本围绕 `conf['log']['sqlite3_file']` 工作
- 量化数据如果误挂到同一库或同一配置链，将面临被现有日志归档策略误伤的风险

因此建议新增独立配置，例如：

- `[quant]`
- `sqlite3_file=/path/to/quant.db`

并明确要求：

- `quant.db` 只承载量化相关表
- 量化库的备份、归档、清理策略独立设计
- 现有 `archive_logs.py` 和 `restore_logs.py` 不得作用于量化库
...
- `logs.db` 或现有 `log.sqlite3_file`
- `quant.db` 或独立 `quant.sqlite3_file`

隔离原则：

- 量化表不得混入现有日志库
- 量化 repository 不得默认复用现有日志库连接配置
- 量化库应有独立初始化入口
- 量化库应有独立备份与恢复脚本
...
- 避免被现有日志归档与删除策略误伤
- 降低日志写入和量化写入之间的锁竞争
- 后续更容易单独迁移 quant 存储到 PostgreSQL

## 9. IM 报告推送设计

目标是支持“交易日每日 2~3 次”报告推送到指定 IM。

当前 IM 主通道只保留飞书自建应用，不再保留其他 IM 的主链路。

原因：

- 已经具备报告推送、对话查询、持仓录入的闭环
- 回调签名、事件订阅和消息发送都已按飞书协议实现
- 继续保留其它 IM 分支会增加配置歧义和维护成本
...
MVP 阶段 SQLite 只有在单 worker、低频写入、独立 `quant.db` 和受控并发前提下才成立。
...
- 用飞书自建应用接入主链路
- 用独立 `quant.db` 做 MVP，但把锁竞争、归档隔离和迁移条件写进设计
- 回测先做简化版
- 自动下单后置
