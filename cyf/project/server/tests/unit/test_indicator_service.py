...
class TestTdSequential:
    ...
    def test_buy_countdown_completes_after_setup_and_allows_skipped_bars(self):
        bars = []
        for i in range(4):
            bars.append(MockBar(_d(i), 100.0, high_price=101.0, low_price=99.0))
        for i in range(9):
            close = 90.0 - i
            bars.append(MockBar(_d(4 + i), close, high_price=close + 0.2, low_price=close - 0.5))
        for i in range(13):
            close = 80.0 - i
            bars.append(MockBar(_d(13 + i), close, high_price=close + 0.2, low_price=close - 0.5))

        results = compute_td_sequential(bars)
        countdown_rows = [row for row in results.values() if row.get("td_buy_countdown") == 13]
        assert countdown_rows
        assert any(row.get("td_signal") == "buy_countdown_complete" for row in countdown_rows)
