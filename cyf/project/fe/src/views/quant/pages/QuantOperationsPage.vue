<template>
  <div class="quant-grid quant-grid--operations">
    <section class="quant-panel quant-panel--compact">
      <div class="quant-panel__header">
        <div>
          <h2>当前持仓</h2>
          <p>状态变更自动同步；可在「IM 通道」页查看私聊通道配置。</p>
        </div>
        <div class="quant-toolbar">
          <el-button :icon="RefreshRight" @click="refreshOperationsPage" :loading="workbench.loading.operations || workbench.loading.positionSummary">刷新</el-button>
        </div>
      </div>
      ...
      <el-table :data="workbench.positionSummary" stripe height="240" class="quant-table" empty-text="暂无持仓记录">
        <el-table-column label="标的" min-width="160">
          <template #default="{ row }">{{ workbench.displaySymbol(row.symbol) }}</template>
        </el-table-column>
        <el-table-column prop="net_quantity" label="净持仓" width="80" />
    ...
    <section class="quant-panel quant-area-editor">
        <div class="quant-panel__header">
          <div>
            <h2>{{ workbench.operationForm.id ? '编辑操作记录' : '新建操作记录' }}</h2>
            <p>在「操作清单」选中行后自动带入此处，可修改状态/价格/备注后保存。</p>
      ...
      <section class="quant-panel quant-area-operations">
        <div class="quant-panel__header">
          <div>
            <h2>操作清单</h2>
            <p>这里登记人工是否采纳、怎么执行、后续结果如何。修改状态（草稿↔已执行）会自动同步持仓。</p>
          </div>
          <div class="quant-toolbar">
...
const refreshOperationsPage = () => {
  void Promise.all([
    workbench.loadOperations(),
